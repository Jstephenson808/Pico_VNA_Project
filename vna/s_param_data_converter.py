...
class CsvSParamDataConverter(SParamDataConverter):
    ...
    def convert_to_s_param_data(self) -> SParameterDataPandas:
        ...
        return SParameterDataPandas(combined_data_frame)
...
class TouchstoneSParamDataConverter(SParamDataConverter):
    ...
    def convert_to_s_param_data(self) -> SParameterDataPandas:
        return self.extract_data_from_touchstone_folder(
            self.touchstone_folder_path, self.experiment_label
        )
    ...
    def extract_data_from_touchstone_folder(self, folder_path, label) -> pd.DataFrame:
        ...
        times = [
            self.get_time_recorded_from_touchstone(os.path.join(folder_path, path))
        ...
        for touchstone_file in touchstone_files:
            ...
            df, times = self.extract_values_from_touchstone_files_to_df(
                path, label, times, df
            )
            i += 1
        return df
    ...
    def s_params_as_list(
        self, network: Network, s_param, phase_flag=False, mag_flag=False
    ):
        ...
        if phase_flag:
            ...
            return [
                matrix[destination_port - 1][origin_port - 1] for matrix in network.s
            ]
    ...
    def extract_values_from_touchstone_files_to_df(
        self, path, experiment_label, times, df: pd.DataFrame = None
    ) -> pd.DataFrame:
