...
def extract_values_from_touchstone_files_to_df(path, experiment_label, times, df:pd.DataFrame=None)->pd.DataFrame:
    ...
    for s_param in s_params:
        complex_values, phase_values, db_values = get_complex_phase_mag_lists(touchstone_network, s_param)
        print(f"{s_param} time {time_touchstone_created.strftime(DateFormats.MILLISECONDS.value)}")
        phase_row = [experiment_id, label, 'phase', s_param, time_touchstone_created.strftime(DateFormats.MILLISECONDS.value)] + phase_values
        db_row = [experiment_id, label, 'magnitude', s_param, time_touchstone_created.strftime(DateFormats.MILLISECONDS.value)] + db_values
