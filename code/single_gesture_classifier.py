...
def extract_report_dictionary_from_test_results(result_dict):
    ...
    return extract_gesture_metric_values(
        result_dict, columns
    )

def test_data_frame_classifier_frequency_window_with_report(data_frame: pd.DataFrame, label:str, frequency_hop:int=mhz_to_hz(100))->pd.DataFrame:
    ...
    return pd.DataFrame.from_dict(f1_scores, orient="index", columns=[x for x in result.keys() if "report" in x])

def test_classifier_from_df_dict(df_dict: {})->pd.DataFrame:

    full_results_df = None
    for label, data_frame in df_dict.items():
        result_df = test_data_frame_classifier_frequency_window_with_report(data_frame, label, frequency_hop=mhz_to_hz(100))
        full_results_df = pd.concat(
            (full_results_df, result_df))
...
def test_classifier_for_all_measured_params(combined_df: pd.DataFrame)->pd.DataFrame:
    ...
    test_dfs = {f"{param.value}_magnitude": all_Sparams_magnitude[
        all_Sparams_magnitude[DataFrameCols.S_PARAMETER.value] == param.value] for param in SParam}
    test_dfs["all_Sparams_magnitude"] = all_Sparams_magnitude
    test_dfs["all_Sparams_phase"] = all_Sparams_phase
    test_dfs.update(
        {f"{param.value}_phase": all_Sparams_phase[all_Sparams_phase[DataFrameCols.S_PARAMETER.value] == param.value]
         for param in SParam})
...
if __name__ == "__main__":
    ...
    pickle_object(full_results_df, os.path.join(get_pickle_path(), "classifier_results"))